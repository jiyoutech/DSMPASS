...
在「系统设置」上传管理端或认证端证书后，无需重启 DSMPASS 套件；新建 HTTPS 连接会自动使用新证书。上传认证端证书后，后台会优先读取证书里的非通配符 DNS SAN，并自动更新 `访问 IP / 域名` 和 `IDP 地址`；通配符证书不会自动改写访问域名。DSM 套件中心的打开按钮不会跟随 DSM Pass 里自动或手动设置的访问域名变化；配置域名后，直接用域名访问 IDP 地址。
