...
```text
go/dist/dsm/DSMPASS-0.8.27-linux-amd64.spk
go/dist/dsm/DSMPASS-0.8.27-linux-arm64.spk
...
管理后台默认启用 HTTPS，并会自动生成 DSMPASS 自签证书。首次访问时浏览器可能提示证书不受信任，需要按内网环境策略信任或继续访问。管理端和认证端可以分别上传证书；如果使用同一张通配符证书，也可以把同一套证书 PEM 和私钥 PEM 分别上传到两端。
...
- `管理端仅允许内网访问`：开启后管理后台只允许本机和内网访问；关闭时不按来源网段限制。
- IDP 入口只允许通过系统生成的 `IDP 地址` 访问；如果用其他 Host 或端口访问 `/idp/...`，后台会直接拒绝。
...
在「系统设置」上传管理端证书后，需要重启 DSMPASS 套件后生效。上传认证端证书后，后台会优先读取证书里的非通配符 DNS SAN，并自动更新 `访问 IP / 域名` 和 `IDP 地址`；通配符证书不会自动改写访问域名。点击「重启认证路由」后，认证入口会加载新证书。DSM 套件中心的打开按钮不会跟随 DSM Pass 里自动或手动设置的访问域名变化；配置域名后，直接用域名访问 IDP 地址。
