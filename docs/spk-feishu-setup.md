...
```bash
DSMPASS_VERSION=0.8.23 make package-spk
```

输出文件在：

```text
go/dist/dsm/DSMPASS-0.8.23-linux-amd64.spk
go/dist/dsm/DSMPASS-0.8.23-linux-arm64.spk
...
管理后台默认启用 HTTPS，并会自动生成 DSMPASS 自签证书。首次访问时浏览器可能提示证书不受信任，需要按内网环境策略信任或继续访问。管理端不需要上传证书；认证端可以上传自己的证书，系统会读取证书里的 DNS 域名并自动同步到认证入口。
...
在「系统设置」上传认证端证书后，后台会优先读取证书里的非通配符 DNS SAN，并自动更新 `访问 IP / 域名` 和 `IDP 地址`。点击「重启认证路由」后，认证入口会加载新证书。DSM 套件中心的打开按钮不会跟随 DSM Pass 里自动或手动设置的访问域名变化；配置域名后，直接用域名访问 IDP 地址。
