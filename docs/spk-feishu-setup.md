...
管理后台默认启用 HTTPS，并会自动生成自签证书。首次访问时浏览器可能提示证书不受信任，需要按内网环境策略信任或继续访问。上传自己的证书后，系统不会自动解析证书域名并替换访问地址；请在初始化配置或系统设置里手动填写与证书匹配的域名。
...
管理端证书上传后需要在「系统设置」里点击「重启管理端」才会生效。重启会让当前后台连接短暂断开；管理端口没有变化时刷新当前页面即可，如果端口已在套件侧调整，需要改用新端口访问。DSM 套件中心的打开按钮不会跟随 DSM Pass 里手动设置的访问域名变化；配置域名后，直接用域名访问管理后台和 IDP 地址。
