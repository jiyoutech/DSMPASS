...
安装、身份源配置、同步规则和系统设置请参阅[完整使用手册](docs/user-guide.md)。
...
当前公开版本为 [`v0.8.47`](https://github.com/jiyoutech/DSMPASS/releases/tag/v0.8.47)。这是不含 OIDC 的传统登录版，飞书、企业微信和钉钉三种身份源可以任选其一；同一套安装不能同时创建多个身份源。

获取与 NAS 架构匹配的安装包：

| NAS 架构 | 文件 |
| --- | --- |
| Intel / AMD | [`DSMPASS-0.8.47-linux-amd64.spk`](https://github.com/jiyoutech/DSMPASS/releases/download/v0.8.47/DSMPASS-0.8.47-linux-amd64.spk) |
| ARMv8 / aarch64 | [`DSMPASS-0.8.47-linux-arm64.spk`](https://github.com/jiyoutech/DSMPASS/releases/download/v0.8.47/DSMPASS-0.8.47-linux-arm64.spk) |

下载后可使用 Release 中的 [`SHA256SUMS`](https://github.com/jiyoutech/DSMPASS/releases/download/v0.8.47/SHA256SUMS) 校验文件完整性。后续版本请以 [GitHub Releases](https://github.com/jiyoutech/DSMPASS/releases) 页面为准。
...
DSM 用户名按 DSM 7 规则最多保留 64 个 Unicode 字符，支持内部空格以及 DSM 允许的标点；冒号、分号、控制字符和 DSM 明确禁止的符号会在自动生成时清洗，管理员手动填写时则会直接提示修改。

如果 DSM Pass 数据库里已经有另一个飞书身份占用了同一个 DSM 用户名，页面会标记为「已被其他身份占用」，管理员需要决定修改哪一条或两条都修改。

无论使用飞书、企业微信还是钉钉，如果用户姓名生成的 DSM 用户名是 `admin`、`root`、`administrator` 或 `administrators`（不区分大小写），系统都会标记为冲突并禁止开通，管理员必须先改成其他 DSM 用户名。

### 部门组处理

DSM 本地群组不支持飞书那种同名部门层级。DSM Pass 的处理规则是：如果遇到部门名重名会先生成临时路径名并标记为冲突，等待管理员确认。

DSM 部门组名最多保留 32 个 Unicode 字符，支持内部空格和允许的 Unicode 字符；`#`、冒号、分号等 DSM 禁用字符会被清洗或拒绝。

同名部门不会自动开通。打开身份源详情时如果还有冲突，页面会弹出冲突处理窗口；管理员可以参考飞书部门路径，把其中任意一个或多个改成最终 DSM 部门组名。冲突部门处理完成前，同步不会继续开通 DSM 用户和成员关系，避免权限落到错误部门。

身份源部门生成的 DSM 群组名如果是 `admin`、`root`、`administrator` 或 `administrators`（不区分大小写），也会被强制标记为冲突，不能直接关联 DSM 系统群组或同步成员。
